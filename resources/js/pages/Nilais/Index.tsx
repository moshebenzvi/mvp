...
export default function Index({ korektor }: { korektor: User }) {
    ...
    // Initialize schools and mapels from user data - moved from handleOkClick to component level
    useEffect(() => {
        if (korektor && korektor.user_profile && korektor.user_profile.guguses) {
            setSekolahs(korektor.user_profile.guguses.sekolahs || []);

            // Extract all unique active mapels from all students
            const mapels: Mapel[] = [];
            const mapelIds = new Set();

            if (korektor.user_profile.guguses.sekolahs) {
                korektor.user_profile.guguses.sekolahs.forEach((sekolah) => {
                    sekolah.siswas.forEach((siswa) => {
                        siswa.nilais.forEach((nilai) => {
                            if (nilai.mapel.active === 1 && !mapelIds.has(nilai.mapel.id)) {
                                mapelIds.add(nilai.mapel.id);
                                mapels.push(nilai.mapel);
                            }
                        });
                    });
                });
